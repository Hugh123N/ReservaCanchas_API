...
//using Acopio.Pesaje.Audit.RestClient;
...
namespace Reserva.Entity.Transactions
{
    public class UnitOfWork<TContext> : IUnitOfWork where TContext : DbContext
    {
        ...
        private TimeZoneInfo TimeZone { get; set; } = null!;

        public UnitOfWork(TContext dbContext, /*IAuditService auditService, */ILogger<UnitOfWork<TContext>> logger, TimeZoneInfo timezoneInfoData)
        {
            _dbContext = dbContext;
            //_auditService = auditService;
            _logger = logger;
            TimeZone = timezoneInfoData;
        }
        ...
        private void UpdateOffsetTimezone()
        {
            ...
            foreach (var entry in _dbContext.ChangeTracker.Entries())
            {
                foreach (var propertyInfo in entry.Entity.GetType().GetTypeProperties().Where(p => p.PropertyType == typeof(DateTimeOffset)))
                {
                    ...
                    propertyInfo.SetValue(entry.Entity, dateTimeOffset.ToOffset(timezone.GetUtcOffset(dateTimeOffset.UtcDateTime)));
                }

                foreach (var propertyInfo in entry.Entity.GetType().GetTypeProperties().Where(p => p.PropertyType == typeof(DateTimeOffset?)))
                {
                    var dateTimeOffset = entry.CurrentValues.GetValue<DateTimeOffset?>(propertyInfo.Name);
                    if (dateTimeOffset.HasValue) propertyInfo.SetValue(entry.Entity, dateTimeOffset.Value.ToOffset(timezone.GetUtcOffset(dateTimeOffset.Value.UtcDateTime)));
                }
            }
        }
