...
{
    public class SearchPagoQueryHandler : SearchQueryHandlerBase<SearchPagoQuery, SearchPagoFilterDto, SearchPagoDto>
    {
        private readonly IRepository<Entity.Models.Pago> _PagoRepository;

        public SearchPagoQueryHandler(
            IMapper mapper,
        ...
        protected override async Task<ResponseDto<SearchResultDto<SearchPagoDto>>> HandleQuery(SearchPagoQuery request, CancellationToken cancellationToken)
        {
            var response = new ResponseDto<SearchResultDto<SearchPagoDto>>();
            ...
            */
            ...
            var PagoDtos = _mapper?.Map<IEnumerable<SearchPagoDto>>(Pagos.Items);

            var searchResult = new SearchResultDto<SearchPagoDto>(
                PagoDtos ?? new List<SearchPagoDto>(),
                Pagos.Total,
