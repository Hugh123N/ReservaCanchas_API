...
{
    public class SearchDistritoQueryHandler : SearchQueryHandlerBase<SearchDistritoQuery, SearchDistritoFilterDto, SearchDistritoDto>
    {
        private readonly IRepository<Entity.Models.Distrito> _DistritoRepository;

        public SearchDistritoQueryHandler(
            IMapper mapper,
        ...
        protected override async Task<ResponseDto<SearchResultDto<SearchDistritoDto>>> HandleQuery(SearchDistritoQuery request, CancellationToken cancellationToken)
        {
            var response = new ResponseDto<SearchResultDto<SearchDistritoDto>>();
            ...
            */
            ...
            var DistritoDtos = _mapper?.Map<IEnumerable<SearchDistritoDto>>(Distritos.Items);

            var searchResult = new SearchResultDto<SearchDistritoDto>(
                DistritoDtos ?? new List<SearchDistritoDto>(),
                Distritos.Total,
