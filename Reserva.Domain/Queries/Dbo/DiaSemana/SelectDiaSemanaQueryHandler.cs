...
namespace Acopio.Pesaje.Domain.Queries.Dbo.DiaSemana
{
    public class SelectDiaSemanaQueryHandler : SearchQueryHandlerBase<SelectDiaSemanaQuery, SelectDiaSemanaFilterDto, SelectDiaSemanaDto>
    {
        ...
        protected override async Task<ResponseDto<SearchResultDto<SelectDiaSemanaDto>>> HandleQuery(SelectDiaSemanaQuery request, CancellationToken cancellationToken)
        {
            ...
            Expression<Func<Reserva.Entity.Models.DiaSemana, bool>> filter = x => true;

            var filters = request.SearchParams?.Filter;


            if (filters?.IdDia.HasValue == true)
                filter = filter.And(x => x.IdDia == filters.IdDia);

            var sorts = new List<SortExpression<Reserva.Entity.Models.DiaSemana>>();

            if (request.SearchParams?.Sort != null)
            {
                foreach (var srt in request.SearchParams.Sort)
                {
                    var property = IQueryableExtensions.GetSortExpression<Reserva.Entity.Models.DiaSemana>(srt.Direction, srt.Property);
            ...
            var DiaSemanaDtos = _mapper?.Map<IEnumerable<SelectDiaSemanaDto>>(DiaSeamanas.Items);
